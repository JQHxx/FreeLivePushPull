...
- (void)startStream:(NSString *)streamUrl {
    ...
    //[self.kit.streamerBase startStream:pushURL];
}
...
- (void) protraitStramCfg {
    ...
    self.kit.cameraPosition = AVCaptureDevicePositionFront;
...
- (void) landscapeStramCfg {
    ...
    self.kit.streamerBase.videoCodec = KSYVideoCodec_AUTO;
    self.kit.vCapDev.outputImageOrientation = UIInterfaceOrientationLandscapeRight;
    self.kit.aCapDev.noiseSuppressionLevel = KSYAudioNoiseSuppress_HIGH;
...
- (void) onStreamStateChange :(NSNotification *)notification {
    if (self.kit.streamerBase){
        NSLog(@"stream State %@", [self.kit.streamerBase getCurStreamStateName]);
    }
    if(self.kit.streamerBase.streamState == KSYStreamStateError) {
        NSLog(@"连接错误");
        _streamStartButton.userInteractionEnabled = YES;
        [_streamStartButton setTitle:@"开启直播" forState:UIControlStateNormal];
        _redPoint.hidden = YES;
        
    }else if (self.kit.streamerBase.streamState == KSYStreamStateConnecting) {
        NSLog(@"连接中");
        _streamStartButton.userInteractionEnabled = NO;
        [_streamStartButton setTitle:@"连接中..." forState:UIControlStateNormal];
        _redPoint.hidden = YES;
        
    }else if (self.kit.streamerBase.streamState == KSYStreamStateConnected) {
        NSLog(@"已连接");
        _streamStartButton.userInteractionEnabled = YES;
        [_streamStartButton setTitle:@"停止直播" forState:UIControlStateNormal];
        _redPoint.hidden = NO;
        
    }else if (self.kit.streamerBase.streamState == KSYStreamStateDisconnecting) {
        NSLog(@"未连接");
        _streamStartButton.userInteractionEnabled = YES;
        [_streamStartButton setTitle:@"开启直播" forState:UIControlStateNormal];
        _redPoint.hidden = YES;
        
    }else if (self.kit.streamerBase.streamState == KSYStreamStateIdle) {
        NSLog(@"未连接");
        _streamStartButton.userInteractionEnabled = YES;
        [_streamStartButton setTitle:@"开启直播" forState:UIControlStateNormal];
        _redPoint.hidden = YES;
...
- (void)streamStartButtonClicked:(id)sender {
    if([self.streamStartButton.currentTitle  isEqual: @"开启直播"]) {
        NSURL *pushURL = [NSURL URLWithString:_pushURL];
        [self.kit.streamerBase startStream:pushURL];
    }
    
    if([self.streamStartButton.currentTitle  isEqual: @"停止直播"]) {
        [self.kit.streamerBase stopStream];
    }
