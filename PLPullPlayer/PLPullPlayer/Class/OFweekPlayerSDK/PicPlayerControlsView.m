...
const CGFloat PicBottomControlsView_HEIGHT = 55.0f;

@interface PicPlayerControlsView() <UIGestureRecognizerDelegate, SDCycleScrollViewDelegate> {
}
...
@property (strong, nonatomic) SDCycleScrollView *picModeView;
...
@implementation PicPlayerControlsView
...
//                         @"http://images.ofweek.com/Upload/News/2018-02/11/yangyang/1518310528573029993.png", nil];
    ...
    _picModeView = [SDCycleScrollView cycleScrollViewWithFrame:self.frame delegate:self placeholderImage:[UIImage imageNamed:@"defaultIcon"]];
    _picModeView.pageControlAliment = SDCycleScrollViewPageContolAlimentCenter;
    [self addSubview:_picModeView];
//    _picModeView.imageURLStringsGroup = picArray;
    _picModeView.userInteractionEnabled = YES;
    _picModeView.translatesAutoresizingMaskIntoConstraints = NO;

    //全视图的点击事件
//    UITapGestureRecognizer *picModeViewTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(picModeViewTap:)];
//    picModeViewTap.delegate = self;
//    [_picModeView addGestureRecognizer:picModeViewTap];

    NSLayoutConstraint *constraint;
    //_picModeView TOP
    constraint = [NSLayoutConstraint constraintWithItem:_picModeView attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeTop multiplier:1.0f constant:0];
    [self addConstraint:constraint];
    //_picModeView LEFT
    constraint = [NSLayoutConstraint constraintWithItem:_picModeView attribute:NSLayoutAttributeLeading relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeLeading multiplier:1.0f constant:0];
    [self addConstraint:constraint];
    //_picModeView Right
    constraint = [NSLayoutConstraint constraintWithItem:_picModeView attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeRight multiplier:1.0f constant:0];
    [self addConstraint:constraint];
    //_picModeView Bottom
    constraint = [NSLayoutConstraint constraintWithItem:_picModeView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeBottom multiplier:1.0f constant:0];
    [self addConstraint:constraint];
}
...
- (void)addTopAndBottomControlsView {
    ...
    constraint = [NSLayoutConstraint constraintWithItem:_bottomControlsView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0f constant:PicBottomControlsView_HEIGHT];
    [self addConstraint:constraint];
    ...
    constraint = [NSLayoutConstraint constraintWithItem:_topControlsView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0f constant:PicBottomControlsView_HEIGHT];
    [self addConstraint:constraint];
    ...
    constraint = [NSLayoutConstraint constraintWithItem:controlsBottomMask attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0f constant:PicBottomControlsView_HEIGHT];
    [_bottomControlsView addConstraint:constraint];
